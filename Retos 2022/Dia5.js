...
function getMaxGifts(giftsCities, maxGifts, maxCities) {
    let subSets = [[]], result = 0
    for (let giftsCity of giftsCities) {
        let copysubSets = [...JSON.parse(JSON.stringify(subSets))]
        for (let j = 0; j < copysubSets.length; j++) {
            copysubSets[j].push(giftsCity)
            let sum = copysubSets[j].reduce((a, b) => a + b, 0)
            if(copysubSets[j].length <= maxCities && 
            maxGifts >= sum) result = Math.max(result, sum)
        }
        subSets = [...subSets,...copysubSets]
    }
    return result
}
