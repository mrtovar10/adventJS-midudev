...
export default function countPackages(carriers, carrierID) {
    ...
    function subordinados (arreglo) {
        arreglo.forEach(value=>{
            for(let i = 0; i<carriers.length;i++){
                if(value === carriers[i][0]) packages = packages + carriers[i][1]
            }
            let otherArr = carriers.filter(e => e[0]===value)[0][2]
            subordinados(otherArr)
        })
    }

    carriers.forEach((arr)=>{
        if(arr[0]===carrierID){
            packages = packages + arr[1]
            subordinados(arr[2])
        }
    })
