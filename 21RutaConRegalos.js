...
export default function canCarry(capacity, trip) {
    ...
    let tiene = 0
    for (let i =1; i<=10; i++) {
        const entrega = trip.filter(value => value[2] === i)
        for (const j of entrega) tiene = tiene - j[0]
        const recoge = trip.filter(value => value[1] === i)
        for (const k of recoge) tiene = tiene + k[0]
        if(tiene > capacity) return false
    }
    return true
}