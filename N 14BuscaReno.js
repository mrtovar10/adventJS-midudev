...
export default function missingReindeer(ids) {
    ...
    let idsWithNull = [];
    sortedIds.forEach((value,index)=> idsWithNull.push(value===index))
    let arr = idsWithNull.filter(e => e === false)
    if(!arr.length) return idsWithNull.length
    return idsWithNull.indexOf(false)
}