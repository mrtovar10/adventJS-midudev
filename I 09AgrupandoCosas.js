...
export default function groupBy(collection, it) {
  ...
  if(typeof(it)==='function'){
    collection.forEach(element => {
      if(!objetoAgrupador.hasOwnProperty(it(element))){
        objetoAgrupador[it(element)] = [element]
      }else{
        objetoAgrupador[it(element)].push(element)
      }
    })
  }else {
    collection.forEach(element => {
      if(!objetoAgrupador.hasOwnProperty(element[it])){
        objetoAgrupador[element[it]] = [element]
      }else{
        objetoAgrupador[element[it]].push(element)
      }
    }) 
  }
